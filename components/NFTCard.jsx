...
const NFTCard = ({ nft, onProfilePage }) => {
  ...
  return (
    ...
    minmd:min-w-256 minlg:min-w-327 bg-transparent rounded-2xl p-4  m-4 minlg:m-8 sm:my-2 sm:mx-2 cursor-pointer hover:scale-105 duration-300"
      >
        <div className="relative w-full h-52 sm:h-36 minmd:h-60 minlg:h-300 rounded-2xl shadow-lg">
...
bg-white bg-opacity-20 backdrop-blur-sm rounded-2xl sm:p-5 p-10 -mt-6 mx-auto"
        >
          ...
          <div className="flexBetween mt-3 minlg:mt-3 flex-row xs:flex-col xs:items-start xs:mt-3">
            <p className="font-poppins dark:text-white text-nft-black font-semibold text-xs minlg:text-lg flexCenter">{nft.price} <span className="normal ml-1">{nftCurrency}</span></p>
            <p className="font-poppins dark:text-white text-nft-black font-semibold text-xs minlg:text-lg ml-2">{shortenAddress(onProfilePage ? nft.owner : nft.seller)}</p>
          </div>
        </div>
